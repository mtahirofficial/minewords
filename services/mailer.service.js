...
class MailerService extends Service {
  ...
  configTransporter() {
    ...
    if (!mailerUser || !mailerPass) {
      console.error(
        "âš ï¸  MAILER_USER or MAILER_PASS environment variables are not set!",
      );
      ...
      throw new Error(
        "Email service configuration error: Missing MAILER_USER or MAILER_PASS",
      );
    }
  ...
  async sendEmail({ to, subject, template, context, attachments }) {
    ...
    if (!to || !subject || !template) {
      throw new Error(
        "Missing required email parameters: to, subject, or template",
      );
    }
    ...
    if (!this._transporter) {
      throw new Error(
        "Email transporter is not configured. Please check your MAILER_USER and MAILER_PASS environment variables.",
      );
    }
